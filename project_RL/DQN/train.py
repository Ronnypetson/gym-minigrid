...
from project_RL.DQN.deep_q import DeepQLearning
...
def train(env, hyperparameters):
    ...
    for episode in range(int(1e4)):
        ...
        agent_fov_img = env.get_obs_render(observation['image'])
        observation = {
            'agent_fov_img': agent_fov_img,
            'direction': direction
        }
        import pdb; pdb.set_trace()
        state = image_parse_observation_to_state(observation)
        action = agent.get_new_action_e_greedly(state)
        done = False

        while not done:
            observation, reward, done, info = env.step(action)
            next_state = image_parse_observation_to_state(observation)
            total_reward += reward
            ...
            if done:
                ...
                if episode % 100 == 0:
                    play(env, agent, image_parse_observation_to_state)
            step += 1
    env.close()
